...
`dsh` 照常在电脑上跑，手机打开一个链接，界面就在那儿了：走自己家的网络，用配对令牌把关。同一个网络里的其他设备进不来。
...
正常情况下一项都不用改。默认配置是安全的，没列出来的一律拒绝。唯一值得知道的是 `pairedNamespaces`，它决定配对过的手机能碰到哪些东西；没写进去的一律只允许本机，所以这个范围只会被有意放宽。完整字段和覆盖方式见 [AGENTS.md](AGENTS.md)。
